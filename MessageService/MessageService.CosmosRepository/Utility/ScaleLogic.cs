...
namespace MessageService.CosmosRepository.Utility
{
    internal static class ScaleLogic
    {
        ...
        public static ScaleOperation ScaleUpCollectionAsync(DocumentClient client, IMetaDataOperator metaDataOperator, string databaseName, string collectionName, int minRu, int maxRu)
        {
            try
            {
                var latestActivty = metaDataOperator.GetLatestScaleUpAsync(databaseName, collectionName).Result;
                    ...
                    if (collection.Id == collectionName)
                    {
                        lock (_lockingObject)
                        {
                            ...
                            if (newRu <= maxRu)
                            {
                                ...
                                metaDataOperator.AddScaleActivityAsync(databaseName, collectionName, (int)newRu, DateTimeOffset.Now);
                ...
                foreach (var collection in collections)
                {
                    if (collection.Id == collectionName)
                    {
                        ...
                        if (currentRu < maxRu)
                        {
                            ...
                            await metaDataOperator.AddScaleActivityAsync(databaseName, collectionName, (int)maxRu, DateTimeOffset.Now);
